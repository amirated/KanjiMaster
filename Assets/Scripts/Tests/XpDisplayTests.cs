...
namespace KanjiRush.Tests
{
    ...
    public class XpDisplayTests
    {
        ...
        // 2. Displays the current TotalXP (compact)
        [Test]
        public void Shows_Current_Total_Xp()
        {
            XpService.Progress.progression.xp.totalXp = 1250;
            Assert.AreEqual("1.3k XP", XpDisplay.Format(XpService.TotalXp));
        }
        ...
        public void Reflects_Updated_Total_Xp()
        {
            ...
            Assert.AreEqual("1k XP", XpDisplay.Format(XpService.TotalXp));

            XpService.AwardSessionCompletionXp(); // +50 via the existing system → 1050
            Assert.AreEqual("1.1k XP", XpDisplay.Format(XpService.TotalXp), "re-reading shows the new value");
        }

        // 4. The display does not calculate XP — Format only formats the stored value
        [Test]
        public void Does_Not_Calculate_Xp()
        {
            Assert.AreEqual("0 XP", XpDisplay.Format(0));
            Assert.AreEqual("10 XP", XpDisplay.Format(10));
            Assert.AreEqual("12.5k XP", XpDisplay.Format(12500));
        }
